...
`pdist`, `diff`, `qdist`, `scale`, `geom_qq`, `geom_qq_line`
...
<a href = "https://vectorposse.github.io/intro_stats/chapter_downloads/13-normal_models.qmd" download>https://vectorposse.github.io/intro_stats/chapter_downloads/13-normal_models.qmd</a>
...
The symbol $\mu$ is the Greek letter *mu* (pronounced "myoo") and it represents a population mean. Unlike the symbol $\bar{y}$ introduced in Chapter 4 as the "sample mean" calculated from data, the population mean $\mu$ is a theoretical mean for an entire population. Similarly, the symbol $\sigma$ is the Greek letter *sigma* and it represents a population standard deviation (in contrast to $s$, the sample standard deviation calculated from data).

Often, we leave those Greek letters out and write more simply,
...
A nice feature of the normal model is that it allows us to see what percentage of the population being modeled lies in certain regions. 
The following image illustrates a rule called the **68-95-99.7 Rule**.^[Some folks call this the "Empirical Rule" but that name is much more boring.] The beautiful thing about this rule is that, if you can remember the name of the rule, you can remember the content of the rule. (To see the figure below from the Quarto document, you will need to copy and paste the link below into a browser tab.)

![Normal Model](https://upload.wikimedia.org/wikipedia/commons/a/a9/Empirical_Rule.PNG)
