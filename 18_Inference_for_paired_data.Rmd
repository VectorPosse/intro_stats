...
We load the standard `mosaic` package as well as the `MASS` package for the `immer` data, the `openintro` package for the `hsb2` data, and the `broom` package for tidy output. 
...
[NOTE: Previous versions of this module used a dataset collected by a famous early pioneer of statistics, Francis Galton, who was the founder of eugenics. As part of his studies on the heritability of physical traits, Galton collected data on the heights of adult children and their parents. Previous versions of this module directed students to run inference to determine if there was a difference between the heights of mothers and their daughters. We've replaced these instructions with the new ones below to avoid giving further credence to racism and eugenics.]

The `hsb2` dataset contains data from a random sample of 200 high school seniors from the "High School and Beyond" survey conducted by the National Center of Education Statistics. It contains, among other things, students' scores on standardized tests in math, reading, writing, science, and social studies. We want to know if students do better on the math test than on the reading test.
