...
The t scores follow a Student t distribution, not the correlations themselves, so we have to calculate the t scores. We use the `mutate` command^[The `mutate` command comes from a package called `dplyr` which is helpfully loaded for you when you load the `mosaic` package.] to compute the t score for each row of `sims`. The number 242 is $n - 2$.
