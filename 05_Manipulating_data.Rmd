...
Putting a minus sign in front of a variable name in the `select` command will remove the variable. Create a data frame called `hf_select4` that removes `Year`, `DayofMonth`, `DayOfWeek`, `FlightNum`, and `Diverted`. (Be careful with the unusual---and inconsistent!---capitalization in those variable names.) In the second part of the code chunk below, type `hf_select4` so that the data frame prints to the screen (just like in all the above examples).
...
(Don't worry about any warnings you might get about "ungrouping output".)
...
3. We `select` the variables `Month` and `DepDelay`, throwing away all other variables that are not of interest to us. (Don't forget to use the `dplyr::select` syntax to make sure we get the right function!)
