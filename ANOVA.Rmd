...
Let's looks at the UIS data structure:
...
```{r}
BECK_test <- tidy(aov(BECK ~ IV, data = BECK_IV))
BECK_test
```
...
pdist("f", df1 = BECK_test$df[1], df2 = BECK_test$df[2],
      q = BECK_test$statistic[1], invisible = TRUE)
```
...
```{r}
BECK_test_untidy <- aov(BECK ~ IV, data = BECK_IV)
