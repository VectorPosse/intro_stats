...
Welcome to R! This module will walk you through everything you need to know to get started using R.

As you go through this module (and all future modules), please read slowly and carefully, and pay attention to detail. Many steps depend on the correct execution of all previous steps, so reading quickly and casually might come back to bite you later.
...
Packages are sets of commands and functions that people all over the world write. These packages extend the capabilities of R and add useful tools. For example, we would like to use the `MASS` package because it includes an interesting data set on patients with malignant melanoma. If you are using R on your own machine, you'll need to type `install.packages("MASS")` if you've never used the `MASS` package before. If you are using RStudio Server, you may not be able to install packages because you may not have admin privileges. If you need a package that is not installed, contact the person who administers your server.
...
Now---assuming you have the `MASS` package installed---type this at the command prompt:
...
You can scroll through the rows by using the numbers at the bottom or the "Next" button. The only thing you can't do here that you can do with `View` is sort the columns.
...
Any additional files you need for your project should be placed in this directory. Find the file `02_Using_R_Markdown.R md`. (If you are coming from the website, it is posted there. If you are a student in a stats course, your teacher will make the file available to you.) Download the file to your computer and then copy or upload it to your project folder.
