...
$$
SE = \sqrt{\frac{p(1 - p)}{n}} = \sqrt{\frac{0.5(1 - 0.5)}{69}}
$$

**Remember that are working under the assumption of the null hypothesis.** This means that we use $p = 0.5$ everywhere in the formula for the standard error. 
...
Both the sample proportion $\hat{p}$ (stored above as `alive_prop`) and the corresponding z-score can be considered the "test statistic". 
...
The test statistic is `r alive_prop`. In other words, `r 100 * alive_prop`% of heart transplant recipients were alive at the end of the study.

The z score is `r alive_z`. The proportion of survivors is about 2.5 standard errors below the null value.
...
The P-value is `r P`. If there were truly a 50% chance of survival among heart transplant patients, there would only be a `r 100 * P`% chance of seeing data at least as extreme as we saw.
...
```{r}
alive_ci <- alive_test %>%
    get_confidence_interval(point_estimate = alive_prop, level = 0.95, type = "se")
alive_ci
```
...
We don't have a list of 1000 values when we use an abstract curve to represent our sampling distribution. Nevertheless, we can find the 2.5th percentile and the 97.5th percentile using the area under the normal curve as we saw in the last two chapters. If we were to do this "manually" with the `qdist` command, it would look like this:

```{r}
qdist("norm", p = c(0.025, 0.975), mean = 0.3478261, sd = 0.06019293, plot = FALSE)
```
...
If you're paying close attention, you might recognize that we discussed these two methods in a previous chapter about confidence intervals. (Method 3 in that chapter was when you had access to the original data.)
