...
This logic is formalized into a sequence of steps known as a *hypothesis test*. In this section, we will introduce a rubric for conducting a full and complete hypothesis test for the sex discrimination example. (This rubric also appears in the [Appendix](https://vectorposse.github.io/intro_stats/Rubric.html).
...
We are trying to ask some question about a population of interest. However, all we have in our data is a sample of that population. The word inference comes from the verb "infer": we are trying to infer what might be true of a population just from examining a sample.
...
The data comes from a group of 48 bank managers recruited for the study. The population of interest is probably all bank managers, probably only in in the U.S. (where the two researchers were based) and only during the 1970s.
...
* Random
  * We have no evidence that this is a random sample of bank managers. We hope that it is representative. If the population of interest is all bank managers in the U.S., then we have some doubts as to how representative this sample is. It is likely that the bank managers were recruited from limited geographic areas based on the location of the researchers, and we know that geography could easily be a confounder for sex discrimination (because some areas of the country might be more prone to it than others). Despite our misgivings, we will proceed on with the analysis, but we will temper our expectations for grand, sweeping conclusions.

* 10%
  * Regardless of the intended population, 48 bank managers is surely less than 10% of all bank managers under consideration in our population.
...
We have some reservations about how generalizable this conclusion is due to the fact that we are lacking information about how representative our sample of bank managers is. We also point out that this experiment was conducted in the 1970s, so its conclusions are not valid for today.
...
The template below is exactly the same as in the [Appendix](https://vectorposse.github.io/intro_stats/Rubric.html) up to the part about confidence intervals which we haven't learned yet.
