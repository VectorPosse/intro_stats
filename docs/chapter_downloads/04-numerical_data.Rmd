...
`mean`, `sd`, `var`, `median`, `sort`, `IQR`, `quantile`, `summary`, `min`, `max`, `favstats`, `geom_histogram`, `geom_point`, `labs`
</div>
...
Notice the placement of the variables. The y-axis is `lwt`, the numerical variable. The x-axis variable is `race`; the groups are placed along the x-axis. This is consistent with other graph types that place the response variable on the y-axis and the predictor variable on the x-axis.
