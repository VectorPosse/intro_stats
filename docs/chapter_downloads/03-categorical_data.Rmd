...
From this table, we know what the raw data for this variable should look like: there should be 152 rows that say "Adelie," 68 rows that say "Chinstrap," and 124 rows that say "Gentoo." It would be very annoying, though, to make that whole dataframe by hand. Fortunately, there are R tools that will create this dataframe for us.
...
In order for the `uncount` function to work correctly, we need to have all the counts in a single column, but since this is a contingency table, our counts are spread out across several columns. To solve this problem, we'll need one more library that we haven't needed before. The `reshape2` library has lots of tools for reshaping data, including one called `melt` that will do what we need.

```{r}
library(reshape2)
melt(penguin_species_sex_tabyl, variable = "species")
```

(Notice the use of `variable = "species"` to provide the label for the new column, which otherwise didn't appear in our tibble or our tabyl. If we didn't have that line, the new column would be labeled `variable`, which is not very useful.)

Now our data is in the form that `uncount` knows how to deal with. And indeed, we can assemble all these steps together into a pipeline. First, we should build the tibble. Then, we should turn the tibble into a tabyl (sorry), then `melt` the tabyl, and finally `uncount` to get back to the raw data (but notice that the counts are now given in a column called `value`). Finally, we should store the result as a new dataframe. 
...
penguin_species_sex_table %>%
  as_tabyl() %>%
  melt(variable = "species") %>%
  uncount(value) -> penguin_species_sex_raw
penguin_species_sex_raw
```

Indeed, this new dataframe looks just like how we wanted it to look.
