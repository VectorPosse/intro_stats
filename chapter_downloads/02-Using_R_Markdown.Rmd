...
You should have downloaded this chapter as a `.Rmd` file in the final step from the last chapter. If you have not downloaded this file yet, please click the following link to do so:
...
If you successfully completed the previous "Introduction to R" chapter, you have created a new project called `02_Using_R_Markdown` in RStudio and you should have placed this downloaded file in it. (If not, please go back to Chapter 1 and complete the steps of creating a new project in RStudio.)

Let's make sure you're in your project.

**Look at the upper right corner of the RStudio screen. Does it say `02_Using_R_Markdown`? If so, congratulations! You are in your project and you can safely skip down to the next section "What is R Markdown?"**
...
The next file (with extension `.Rmd`) is your R Notebook file. It is really nothing more than a plain text file, although when you open it in RStudio, some magic allows you to see the output from the code chunks you run.

Finally, you have a file with extension `.nb.html`. That is the pretty output file generated when you hit the "Preview" button. (If you happen to see other files in your project folder, you should ignore those and not mess with them.)
