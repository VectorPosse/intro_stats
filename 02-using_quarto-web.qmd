...
No R functions are introduced here, but Quarto syntax is explained.
...
If you are using RStudio through a web browser, you do not need to install any packages. (Any packages you need should already be installed by the server administrators.)

If you are using R and RStudio on your own machine instead of accessing RStudio through a browser, you'll need to type the following commands at the Console:
...
<a href = "https://vectorposse.github.io/intro_stats/chapter_downloads/02-using_quarto.qmd" download>https://vectorposse.github.io/intro_stats/chapter_downloads/02-using_quarto.qmd</a>

The file is now likely sitting in a Downloads folder on your machine (or wherever you have set up for web files to download). If you are logged into RStudio on a web browser, go to the Files tab and click the "Upload" button. From there, leave the first box alone ("Target directory"). Click the "Choose File" button and navigate to the folder on your machine containing the file `02_using-quarto.qmd`. Select that file and click "OK" to upload the file. Then you will be able to open the file in RStudio simply by clicking on it. If you have installed R and RStudio on your own machine, you will need to move the file from your Downloads folder into the `intro_stats` project directory you created at the end of the last chapter. (Again, if you haven't created the `intro_stats` project, please go back to Chapter 1 and follow the directions for doing that.) Moving files around is most easily done using File Explorer in Windows or the Finder in MacOS.
...
There is a button in the toolbar right above the text that says "Render". Go ahead and push it. See what happens.
...
At first, you'll want to work back and forth between the Quarto document and the HTML file to get used to how the formatting in the plain text file get translated to output in the HTML file. After a while, you will look at the HTML file less often and work mostly in the Quarto file, only rendering the final HTML output when you are finished and ready to produce your final draft.


## Literate programming

Quarto is one way to implement a "literate programming" paradigm. The concept of literate programming was famously described by Donald Knuth, an eminent computer scientist. The idea is that computer programs should not appear in a sterile file that's full of hard-to-read, abstruse lines of computer code. Instead, functional computer code should appear interspersed with writing that explains the code.
...
If you want sub-items, indent at least two spaces and use a plus sign followed by a space.
...
There are many more formatting tricks available. For a good resource on all Quarto stuff, click on [this link](https://quarto.org/docs/authoring/markdown-basics.html). And notice in the previous sentence the syntax for including hyperlinks in your document.
...
The next file (with extension `.Rmd`) is your Quarto file. It's the file you're looking at right now. It is really nothing more than a plain text file, although when you open it in RStudio, some magic allows you to see the output from the code chunks you run.
