...
[You should type `?heartTr` at the Console to read the help file and use `View` to look at the spreadsheet view of the data.]
...
Then, using our computation of `SE`, we get the z-score.

```{r}
z <- (surv_test_tidy$estimate - 0.5)/SE
z
```
...
Once this is done (in the step "Compute the test statistic"), all remaining steps of the rubric stay exactly the same except that you'll use `surv_test_count_tidy` instead of `surv_test_tidy`.
