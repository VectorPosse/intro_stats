...
**Do not under any circumstances attempt to convert `heartTr$survived` to a factor variable using the `factor` command!** It is already a factor variable. Applying the `factor` command the way you have in past assignments will break a variable that is already coded as a factor variable. This is one of the most common mistakes students make when working with factor variables. This is why the `str` command is so important. You always need to check your categorical data to see if it's already a factor variable! (If not, then of course we would recode it the way you've been taught.)
...
Making sure that we refer from now on to the `heartTr_2` data frame and not the original `heartTr` data frame:
...
```{r}
surv_test <- tidy(binom.test(heartTr_2$survived, p = 0.5, success = "alive"))
surv_test
```
...
As seen before, the `tidy` command from the `broom` package makes everything neat and gives us easy access to the various pieces of the output. What this command does not give us is a z-score. So we calculate it directly using the `estimate` ($\hat{p}$) and the mean and standard error of the null distribution.

**Remember that are working under the assumption of the null hypothesis.** This means that we use $p = 0.5$ everywhere in the formula for the standard error. In other words,

$$z = \frac{(\hat{p} - p)}{\sqrt{\frac{p (1 - p)}{n}}} = \frac{(0.348 - 0.5)}{\sqrt{\frac{0.5 (1 - 0.5)}{69}}} =  -2.53.$$

Either $\hat{p}$ or $z$ could be considered the test statistic.^[And there's even a third possibility: the tidy output seems to be considering the raw number of successes (24) as the test statistic.] If we use $\hat{p}$ as the test statistic, then we're considering the null model to be
...
The P-value can also be computed as part of the `binom.test` function:

```{r}
surv_test$p.value
```

The `binom.test` function assumes by default that you are performing a two-sided test. Therefore, if we want a one-sided P-value, we have to divide this answer by 2:

```{r}
surv_test$p.value / 2
```

You might notice that this is a little different from the P-value we computed directly from the normal model. Under the hood, the `binom.test` command is not quite using the same method we're using. Therefore, the P-value is different from the one generated from our normal model. Nevertheless, the two methods are close, so you should come to the same conclusion (i.e., reject the null or fail to reject the null).
...
We are 95% confident that the true percentage of heart transplant recipients who survive is captured in the interval (`r 100 * surv_test$conf.low`%, `r 100 * surv_test$conf.high`%).
...
```{r}
surv_test_count <- tidy(binom.test(24, n = 69))
surv_test_count
```

Once this is done (in the step "Compute the test statistic"), all remaining steps of the rubric stay exactly the same except that you'll use `surv_test_count` instead of `surv_test`.
...
```{r}
surv_test_prop <- tidy(binom.test(round(69*0.348), n = 69))
surv_test_prop
```

Once this is done (in the step "Compute the test statistic"), all remaining steps  of the rubric stay exactly the same except that you'll use `surv_test_prop` instead of `surv_test`.
