...
You may just have a summary of the total number of successes and failures. In our heart transplant example, we had 24 patients who survived and 45 who died. If that's all we know, we can run the `prop.test` command as follows:

```{r}
surv_test_count <- tidy(prop.test(24, n = 69))
surv_test_count
```
...
If you are given the percentages of successes and/or failures in your data, you'll have to convert them to whole number totals. You might be told that of 69 patients who received a heart transplant, only 34.8% survived. In that case, we can run the `prop.test` command as follows:

```{r}
surv_test_prop <- tidy(prop.test(round(69*0.348), n = 69))
surv_test_prop
```
