...
$$z = \frac{(\bar{y} - \mu)}{\sigma/\sqrt{n}} = \frac{(110.20 - 114)}{\sigma/\sqrt{4}}.$$
...
$$t = \frac{(\bar{y} - \mu)}{s/\sqrt{n}} = \frac{(110.20 - 114)}{13.06/\sqrt{4}} = -0.58.$$
...
```{r}
teacher2 <- filter(teacher, total > 40000)
```
...
```{r}
total_test <- tidy(t.test(teacher2$total, mu = 63024))
...
```{r}
pdist("t", df = total_test$parameter, q = c(-t, t), invisible = TRUE)
```

Commentary: The `pdist` command is the same as always except now we use a `"t"` model instead of `"norm"`. We know there are `r NROW(teacher2) - 1` degrees of freedom, but we might as well use the value stored in the `t.test` output to make our code reusable. Finally, note that `q = c(-t, t)` will plot in both tails of the distribution since we're running a two-sided test. Of course, since the t-score is crazy huge, it doesn't actually appear in the resulting graph.

### Calculate and report the P-value.

```{r}
P <- total_test$p.value
P
```

$P < 0.001$

Commentary: When the P-value is this small, it is traditional to report simply $P < 0.001$.
...
```{r}
2 * pdist("t", df = 60, q = t, plot = FALSE)
```
