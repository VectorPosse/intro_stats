...
The numbers in these three cells are the "observed" values (usually denoted by the letter $O$). What are the expected counts? Well, since there are `r NROW(cyl)` cars and there are 3 categories, we would expect there to be `r NROW(cyl)/3` cars at each level. This is the "expected" count (usually denoted by the letter $E$).
...
We've actually run the test twice, once without `tidy` and once with `tidy`. We'll use the `tidy` output when we can, but not everything we'll need to use below appears in the `tidy` output.

### Plot the null distribution.

```{r}
pdist("chisq", df = cyl_test_tidy$parameter, q = cyl_test_tidy$statistic, 
      invisible = TRUE)
```

Commentary: The `pdist` command is familiar, but since we are using a chi-square model and not a normal model, we use `"chisq"` and not `"norm"`. This also requires us to specify the number of degrees of freedom. (Without it, R does not know which chi-square distribution to use.) We know there are 2 degrees of freedom, but since the result is stored in `cyl_test_tidy$parameter`, we might as well use that. It makes our code more portable than if we had manually typed `df = 2`. (The next time we need to run this test, we might not have two degrees of freedom anymore.)

### Calculate the P-value.

```{r}
1 - pdist("chisq", df = cyl_test_tidy$parameter,
...
$H_{0}: p_4 = 0.35, p_6 = 0.4, p_8 = 0.25$
