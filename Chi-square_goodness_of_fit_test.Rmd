...
Commentary: The `chisq.test` command is a little unusual in that you first have to make a frequency table, and then run the test on the table (not the raw data). The $\chi^{2}$ test statistic is, of course, the same value we computed manually by hand earlier.
