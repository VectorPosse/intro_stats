...
It is interesting to note that when there are only two categories, one can still run a chi-square test and the results are more or less equivalent to the one-proportion test.
...
We load the standard `mosaic` package and the `openintro` package for the `hsb2` data.
...
First of all, what is the actual distribution of cylinders in our data? Let's convert `cyl` to a factor variable first, and then we'll count. (Since the levels are already called "4", "6", and "8", we do not need to specify `levels` or `labels`.)
...
The numbers in these three cells are the "observed" values (usually denoted by $O$). What are the expected counts? Well, since there are `r NROW(cyl)` cars and there are 3 categories, we would expect there to be `r NROW(cyl)/3` cars at each level. This is the "expected" count (usually denoted by $E$).
...
We know that even if the true distribution were `r NROW(cyl)/3` in each cell, we would not see that exactly if we collect a sample of `r NROW(cyl)` cars. (In fact, the "true" distribution is physically impossible because `r NROW(cyl)/3` is not a whole number!) So what kinds of numbers could we get?

Let's do a quick simulation to find out. We will use the `sample` command from the `mosaic` package.
...
Under the assumption of the null, there should be an equal chance of seeing 4, 6, or 8 cylinders. The `sample` command below takes the `levels` of `cyl` ("4", "6",  or "8") and grabs them at random until there are `r NROW(cyl)` values. We also have to specify `replace = TRUE` to let `sample` know that there can (and should!) be more than one car in any given level.

```{r}
sample(levels(cyl), 32, replace = TRUE)
```

Let's do this a couple of times, this time organizing the output into a table:

```{r}
table(sample(levels(cyl), 32, replace = TRUE))
table(sample(levels(cyl), 32, replace = TRUE))
table(sample(levels(cyl), 32, replace = TRUE))
```
...
```{r}
chisq(table(sample(levels(cyl), 32, replace = TRUE)))
chisq(table(sample(levels(cyl), 32, replace = TRUE)))
chisq(table(sample(levels(cyl), 32, replace = TRUE)))
```
...
```{r}
sims <- do(5000) * chisq(table(sample(levels(cyl), 32, replace = TRUE)))
head(sims)
```
...
The sample is a set of `r NROW(cyl)` cars from a 1974 Motor Trends magazine. The population is all cars from 1974. (We would not want to assume that the number of cylinders in cars today would be similar to the distribution in 1974.)
...
* Expected cell counts
    - This condition says that under the null, we should see at least 5 cars in each category. We expect `r NROW(cyl)/3` in each cell, so this condition is met.
...
```{r}
cyl_table <- table(cyl)
cyl_test <- chisq.test(cyl_table)
cyl_test$statistic
```

Commentary: The `chisq.test` command is a little unusual in that you first have to make a table, and then run the test on the table (not the vector of cylinder values.) The $\chi^{2}$ test statistic is, of course, the same value we computed manually by hand earlier.

### Plot the null distribution.

```{r}
pdist("chisq", df = cyl_test$parameter, q = cyl_test$statistic)
```

Commentary: The `pdist` command is familiar, but since we are using a chi-square model and not a normal model, we use `"chisq"` and not `"norm"`. This also requires us to specify the number of degrees of freedom. (Without it, R does not know which chi-square distribution to use.) We could just type $df = 2$ here, but the degrees of freedom is stored in the output of the `chisq.test` command in the `parameter` variable.

### Calculate the P-value.

```{r}
1 - pdist("chisq", df = cyl_test$parameter, q = cyl_test$statistic,
         plot = FALSE)
cyl_test$p.value
```

Commentary: Values that are as extreme or even more extreme than the test statistic are in the right tail. If we use `pdist`, remember that it always shades to the left by default, so we have to subtract the output from 1 to get the correct P-value. Also remember to add `plot = FALSE` as we don't really need to look at the same picture again. (Ignore the `X-squared` above the P-value. This is carried over from the name assigned to `cyl_test$statistic` and isn't removed by the `pdist` command.)

Alternatively, you can see that the `chisq.test` command gives us the exact same value, storing the result in the `p.value` variable of the output.
...
```{r}
cyl_test_2 <- chisq.test(cyl_table, p = c(0.35, 0.4, 0.25))
```
...
```{r}
cyl_test_3 <- chisq.test(cyl_table, p = c(6, 4, 3), rescale.p = TRUE)
```
