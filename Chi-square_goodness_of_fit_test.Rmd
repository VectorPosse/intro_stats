...
Commentary: Instead of just displaying the frequency table, we save it as `cyl_tally` because we'll need it later.
...
cyl_tidy <- tidy(cyl_test)
cyl_tidy$statistic
```

The value of $\chi^{2}$ is `r cyl_tidy$statistic`.

Commentary: The `chisq.test` command is a little unusual in that you have to run the test on the frequency table and not the raw data.

The $\chi^{2}$ test statistic is, of course, the same value we computed manually by hand earlier.

We've actually run the test in two steps, the first using `chisq.test` alone (without `tidy`) and the second applying `tidy` to the output of the `chisq.test` command. We'll use the `tidy` output when we can, but we'll also need some things that are part of the `chisq.test` output, but don't get stored in the tidy output.

### Plot the null distribution.

```{r}
pdist("chisq", df = cyl_tidy$parameter,
      q = cyl_tidy$statistic, 
      invisible = TRUE)
```
...
```{r}
P <- 1 - pdist("chisq", df = cyl_tidy$parameter,
          q = cyl_tidy$statistic,
          plot = FALSE)
P
```
