...
[You should type `?birthwt` at the Console to read the help file and use `View` to look at the spreadsheet view of the data.]
...
Once this is done (in the step "Compute and report the test statistic"), all remaining steps of the rubric stay exactly the same except that you'll use `low_race_test_manual_tidy` instead of `low_race_test_manual_tidy`.
