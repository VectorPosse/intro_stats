...
The intercept is `r wrist_wt_tidy$estimate[1]` and the slope is `r wrist_wt_tidy$estimate[2]`.
...
The slope $b_{1}$ is always interpretable. This model predicts that one unit of increase in the x-axis corresponds to a change of `r wrist_wt_tidy$estimate[2]` units in the y-direction. Let's phrase it this way:

> The model predicts that an increase of one cm in wrist diamater corresponds to a weight increase of `r wrist_wt_tidy$estimate[2]` kg.

The intercept $b_{0}$ is a different story. There is always a literal interpretation:

> The model predicts that someone with a wrist diameter of zero will weigh `r wrist_wt_tidy$estimate[1]` kg.
...
```{r}
wrist_wt_aug <- augment(lm(wgt ~ wri.di,  data = bdims))
```
...
A more advanced statistics course might discuss the other model summaries present in the `glance` output. The $R^{2}$ value is stored in `wrist_wt_glance$r.squared`. Its value is `r wrist_wt_glance$r.squared`. We will word it this way:

> `r 100*wrist_wt_glance$r.squared`% of the variability in weight can be explained by variability in wrist diameter.
...
We are 95% confident that the true slope of the relationship between wrist diameter is captured in the interval (`r CI$conf.low[2]`,  `r CI$conf.high[2]`).
