...
First, let's get the contingency table for the relationship between `low` and `race` and assign it to a variable for future use:
...
[Type `?birthwt` at the Console to read the help file.]
...
```{r}
low_race_tidy <- tidy(low_race_test)
low_race_tidy$statistic
```
...
```{r}
pdist("chisq", df = low_race_tidy$parameter,
      q = low_race_tidy$statistic,
      invisible = TRUE)
```

Commentary: We know there are 2 degrees of freedom, but since the result is stored in `low_race_tidy$parameter`, we might as well use that. It makes our code more portable than if we had manually typed `df = 2`. (The next time we need to run this test, we might not have two degrees of freedom anymore.)

### Calculate and report the P-value.

```{r}
P <- 1 - pdist("chisq", df = low_race_tidy$parameter,
               q = low_race_tidy$statistic,
               plot = FALSE)
P
```
