...
We load the standard `mosaic` package. We also use the `gmodels` package for the `CrossTable` command, the `MASS` package for the `birthwt` data, and the `openintro` package for the `smoking` data. The `broom` package will give us tidy output.
...
After converting to factor variables, look at the contingency table for the relationship between `low` and `race` in the `birthwt` data from the `MASS` package. The new option in the `CrossTable` command is `expected = TRUE`:
...
CrossTable(low_race$low, low_race$race,
           prop.r = FALSE, prop.c = TRUE, prop.t = FALSE, prop.chisq = FALSE,
           expected = TRUE)
```

To see how these expected counts are computed, look at the marginal distribution along the right. If race is ignored, there were 59 low birth weight babies and 130 normal weight babies out of 189 total babies. 59 of 189 is `r 59/189` or `r 100 * 59/189`%, and 130 of 189 is `r 130/189` or `r 100 * 130/189`%.

Now, if low birth weight and race are truly independent, it shouldn't matter if the mothers were white, black, or some other race. In other words, of 96 white mothers, we should still expect `r 100 * 59/189`% of them to have low birth weight babies and `r 100 * 130/189`% of them to have normal weight babies. `r 100 * 59/189`% of 96 is `r 59/189 * 96`. Look in the contingency table above and note that this is the expected cell count for low birth weight babies among white women. Also, `r 100 * 130/189`% of 96 is `r 130/189 * 96`. Again, this is listed in the contingency table in the cell for normal weight babies among white mothers. The same analysis can be done for the next two columns as well.
...
This is equivalent to the explanation in the previous paragraph. Using low birth weight babies among white mothers as an example, $C/n$ is $59/189$ which is `r 59/189`. Then we multiply this by the row total $R = 96$ to get

$$ \left(\frac{C}{n}\right) R = \frac{C \times R}{n} = \frac{59 \times 96}{189} =  29.96825.$$
...
In our example we have 2 columns ("Yes", "No") and 3 rows ("White", "Black", "Other"); therefore,
...
```{r}
CrossTable(low_race$low, low_race$race,
           prop.r = FALSE, prop.c = TRUE, prop.t = FALSE, prop.chisq = TRUE,
           expected = TRUE)
```
...
```{r}
low_race_test <- chisq.test(low_race$low, low_race$race)
low_race_test_tidy <- tidy(low_race_test)
low_race_test$statistic
```

The value of $\chi^{2}$ is `r low_race_test$statistic`.

Commentary: This just reproduces the value of $\chi^{2}$ we already saw in the summary below the contingency table.

Also note that the `chisq.test` function is not supported by the `mosaic` package, which means that we can't use the "tilde" notation as we've done in the past. In other words, the command
...
Commentary: We know there are 2 degrees of freedom, but since the result is stored in `low_race_test_tidy$parameter`, we might as well use that. It makes our code more portable than if we had manually typed `df = 2`. (The next time we need to run this test, we might not have two degrees of freedom anymore.)

### Calculate the P-value.

```{r}
1 - pdist("chisq", df = low_race_test_tidy$parameter,
          q = low_race_test_tidy$statistic,
          plot = FALSE)
```
