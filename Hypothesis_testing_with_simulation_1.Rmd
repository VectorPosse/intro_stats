...
```{r}
tally(low ~ smoke, data = low_smoke, margins = TRUE, format = "percent")
```
...
3. You can safely ignore the word `TRUE` in the output. That's just R being weird. We could get rid of it using the `unname` command if we wanted to:
