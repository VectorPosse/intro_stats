...
The `do` command from the `mosaic` package allows us to repeat this simulation process any number of times. From an earlier module, we learned that the number of simulations needs to be sifficiently high to make sure we have a good representative set of outcomes. The results will be gathered together in a data frame that we will call `sims`.
