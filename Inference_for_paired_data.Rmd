...
```{r}
immer_d_noM <- filter(immer_d, Loc != "M")
```

Here is the new histogram and QQ plot:

```{r}
ggplot(immer_d_noM, aes(x = d)) +
    geom_histogram(binwidth = 10)
```

```{r}
ggplot(immer_d_noM, aes(sample = d)) +
    geom_qq()
```
...
The sample consists of `r NROW(immer_d_noM)` plots representing five varieties of barley grown in five different locations. (There were 30 plots across six locations, but remember that we removed the observations from one of the locations to get rid of the extra mode.) The population is all possible locations at which we might try growing these varieties of barley.
...
```{r}
barley_test <- tidy(t.test(immer_d_noM$Y2, immer_d_noM$Y1, paired = TRUE))
...
```{r}
barley_test2 <- tidy(t.test(immer_d_noM$d, mu = 0))
...
```{r}
immer_d_no_outlier <- filter(immer_d_noM, d < 20)
