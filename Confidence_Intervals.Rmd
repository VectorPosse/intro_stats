...
```{r}
test_3a <- tidy(prop.test(hsb2$schtyp))
test_3a
```
...
```{r}
test_3b <- tidy(prop.test(hsb2$schtyp, success = "private"))
test_3b
```
...
```{r}
test_3c <- tidy(prop.test(hsb2$schtyp, success = "private",
                          conf.level = 0.9))
test_3c
```
...
```{r}
program <- tidy(prop.test(hsb2$prog, success = "vocational"))
program
```
...
For each of the 100 simulated intervals, most of them (the black ones) do capture the true value of 0.64 (the blue vertical line). Occassionally they don't (the red ones). We expect 5 red intervals, but since randomness is involved, it won't necessarily be exactly 5. (Here there were only 3 bad intervals.)
