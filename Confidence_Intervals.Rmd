...
```{r}
test_1 <- tidy(prop.test(212, n = 326))
test_1
```

For now, ignore everything but the two numbers of interest: the low and high endpoints of our confidence interval given in the `conf.low` and the `conf.high` columns. Here is how we report this inline:

We are 95% confident that the true percentage of those who support the new initiative is captured in the interval (`r 100 * test_1$conf.low`%, `r 100 * test_1$conf.high`%).
...
```{r}
test_2 <- tidy(prop.test(round(326*0.65), n = 326))
test_2
```
...
```{r}
test_3a <- tidy(prop.test(~ schtyp, data = hsb2))
test_3a
```
...
```{r}
test_3b <- tidy(prop.test(~ schtyp, data = hsb2, success = "private"))
test_3b
```

We are 95% confident that the true percentage of U.S. high school seniors who attend private school is captured in the interval (`r 100 * test_3b$conf.low`%, `r 100 * test_3b$conf.high`%).
...
```{r}
test_3c <- tidy(prop.test(~ schtyp, data = hsb2,
                          success = "private",
                          conf.level = 0.9))
test_3c
```
...
```{r}
program <- tidy(prop.test(~ prog, data = hsb2, success = "vocational"))
program
```

### State (but do not overstate) a contextually meaningful interpretation.

We are 95% confident that the true percentage of U.S. high school seniors who attend a vocational program is captured in the interval (`r 100 * program$conf.low`%, `r 100 * program$conf.high`%).
...
Let's see how confidence intervals work using simulation.

In order to simulate, though, we'll have to pretend temporarily that we know a true population parameter. Let's use the example of a candidate who has the support of 64% of voters. In other words, $p = 0.64$. We go out and get a sample of voters, let's say 50. From that sample we get a sample proportion $\hat{p}$ and construct a 95% confidence interval by adding and subtracting 1.96 standard errors from the value of $\hat{p}$. Most of the time, 64% (the true value!) should be in our interval. But sometimes it won't be. We can get a "freak" sample that is far away from 64%, just by pure chance alone. (Perhaps we accidentally run into a bunch of people who oppose our candidate.)
...
```{r}
confint_1 <- tidy(prop.test(nflip(50, prob = 0.64), n = 50))
confint_1
```

All we need to do for simulation purposes is to introduce the `do` command to do this a bunch of times:

```{r}
confint_100 <- do(100) * tidy(prop.test(nflip(50, prob = 0.64), n = 50))
head(confint_100)
```


Each sample gives us a slightly different estimate, and therefore, a different confidence interval as well. (Depending on your screen size and resolution, you may have to click the right-facing black arrow in the output above to scroll over to where the `conf.low` and `conf.high` are listed.)
